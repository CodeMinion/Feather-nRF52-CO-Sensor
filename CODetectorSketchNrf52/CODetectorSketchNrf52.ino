...
const char* DEVICENAME = "CO HP%X%X";
...
const int UUID16_CHR_CO_SENSE_MEASUREMENT = 0x4A39;

const int VAL_CO_DETECTED = 0x01; // CO detected 

volatile int coDetectedValue = 0x00;

BLEService        hess = BLEService(UUID16_SVC_HOME_ENV_SENSE);
BLECharacteristic cosc = BLECharacteristic(UUID16_CHR_CO_SENSE_MEASUREMENT);
...
const int iCOInterruptPin = A2;

/**
   Notification task informaiton
*/
static TaskHandle_t _notifyCOValueHandle;
static TaskHandle_t _taskToNotify;

uint32_t _notifyCOValueStackSize = 512;
void TaskNotifyCO(void * pvParameters);
...
void setup()
{
  ...
  DEBUG_PRINTLN("Setting up CO Sensor");
  DEBUG_PRINTLN("-----------------------\n");
  ...
  DEBUG_PRINTLN("Configuring the CO Sensor Service");
  setupLS();
...
void cccd_callback(BLECharacteristic& chr, uint16_t cccd_value)
{
  ...
  if (chr.uuid == cosc.uuid)
  {
    if (chr.notifyEnabled())
    {
      DEBUG_PRINTLN("CO Sensing Measurement 'Notify' enabled");
      // Notify current CO detection.
      notifyCODetectionValue(coDetectedValue);
    }
    else
    {
      DEBUG_PRINTLN("CO Sensing Measurement 'Notify' disabled");
    }
  }
...
void setupLS(void)
{
  ...
  // ---------------------------- ------  ----------- ----------
  // CO Sense Measurement         0x2A39  Mandatory   Notify
  hess.begin();
  ...
  cosc.setProperties(CHR_PROPS_NOTIFY);
  cosc.setPermission(SECMODE_OPEN, SECMODE_NO_ACCESS);
  cosc.setFixedLen(2);
  cosc.setCccdWriteCallback(cccd_callback);  // Optionally capture CCCD updates
  cosc.setUserDescriptor("CO Sense Measurement. A value of greater than 0 means detected.");
  cosc.begin();
  uint8_t lsdata[1] = { 0 }; // Set the characteristic to use 8-bit values, with the sensor connected and detected
  cosc.notify(lsdata, 1);                   // Use .notify instead of .write!
...
void setupCOSensor()
{
    pinMode(iCOInterruptPin, INPUT);
}

void setupCODectectionInterrupts()
{
    attachInterrupt(digitalPinToInterrupt(iCOInterruptPin), coChanged, CHANGE);
}
...
void coChanged()
{
  ...
  */
  ...
  vTaskNotifyGiveFromISR( _notifyCOValueHandle, &xHigherPriorityTaskWoken);
...
void TaskNotifyCO(void * pvParameters)
{
  ...
  for (;;)
  {
    ...
    DEBUG_PRINTLN("CO Task Notified");
    ...
    if ( ulNotifiedValue > 0 )
    {
      ...
      {
        DEBUG_PRINTLN("Notifying CO Data");
        ...
        DEBUG_PRINT("CO: "); DEBUG_PRINTLN(coReading);

        coDetectedValue = coReading;
        notifyCODetectionValue(coDetectedValue);
      }
      else
      {
...
int getCOReading()
{
  ...
  coReading = digitalRead(iCOInterruptPin) != HIGH ? 0x01 : 0x00;
  
  return coReading;
}

void notifyCODetectionValue(int value)
{
  uint8_t coData[1] = {value};
  if (cosc.notify(coData, sizeof(coData)))
  {
    DEBUG_PRINT("CO Value Measurement updated to: ");
    DEBUG_PRINTLN(coData[0]);
  }
  else
  {
